#!/bin/bash
# proxy-ctl — Scaleway/Linode IPv6 SOCKS5 proxy farm manager (HIDE-HOSTNAME variant)
# 3proxy egress is forced IPv6-only (-6) to prevent IPv4 fallback,
# so destination sites never see the VPS IPv4 nor its Linode rDNS hostname.
# Tradeoff: sites with no AAAA record are unreachable through these proxies.
...
usage() {
  cat <<EOF
proxy-ctl — Scaleway/Linode IPv6 SOCKS5 proxy manager (HIDE-HOSTNAME variant)
...
# Returns ALL unique /64 prefixes derived from global IPv6 addresses on iface.
detect_ipv6_prefixes_from_iface() {
  local iface=$1
  ip -6 addr show dev "$iface" scope global -temporary 2>/dev/null \
    | awk '/inet6/ && !/deprecated/ {print $2}' \
    | cut -d/ -f1 \
...
#   2. Otherwise, auto-detect ALL /64 prefixes from the default interface.
get_prefixes() {
  ...
  local iface result
  iface=$(detect_iface)
  result=$(detect_ipv6_prefixes_from_iface "$iface")
  [ -n "$result" ] || die "no global IPv6 prefix found on $iface"
  echo "$result"
}
...
clean_prefix() {
  echo "$1" | sed -E 's#::(/[0-9]+)?$##' | sed -E 's#:0:0:0:0(/[0-9]+)?$##'
}
...
EOH
  ...
  if [ -s "$STATE_FILE" ]; then
    ...
    while IFS=$'\t' read -r ipv4 port user pass ipv6; do
      ...
      echo "socks -6 -a -p${port} -i${ipv4} -e${ipv6}" >> "$CONF_FILE"
    done < "$STATE_FILE"
  fi
...
while IFS= read -r ip6; do
  ...
  ip -6 addr replace \${ip6}/128 dev \$IFACE \\
    valid_lft forever preferred_lft forever nodad 2>/dev/null || true
done < $IPV6_LIST
EOR
